...
function salvar(){
    ...
    valuesFiltrados.push(newElement.value);
...
//
function filtro(){
    valorMin = document.getElementById("minimo").value;
    valorMax = document.getElementById("maximo").value;
    if (valuesFiltrados[0] >= valorMin && valuesFiltrados[0] <= valorMax){
        document.getElementById("print2").innerHTML += 
        `<tr>
            <td>${entriesData[0].name}</td>
            <td>${entriesData[0].date}</td>
            <td>R$${entriesData[0].value}</td>
        </tr>`;
    }
}
